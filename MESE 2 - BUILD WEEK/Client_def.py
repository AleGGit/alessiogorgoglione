...
while True:
    ...
    if message == "e":
        ...
        my_sock.sendall(message.encode())  #Avvia la condizione "1" del menù
        data = my_sock.recv(1024) 
        if not data:  
            break
        print(data.decode('utf-8'))  
    elif message == "2":
        path = input("Inserisci il percorso: ")  # Richiede all'utente di inserire un percorso dopo aver scelto l'opzione "2" del menù
        ...
        print(data)  
        while True:
            command = input("Inserisci un comando: ")  # Richiede all'utente di inserie in imput un comando dopo essere entrato nella condizione "3"
